...
Intrinsic layout primitives for shadcn/ui. Source: `registry/new-york/ui/`. Docs: `/`, `/intrinsic`, `/tailwind`, `/stack`, `/cluster`, `/aside`, `/switcher`, `/grid`, `/center`, `/cover`, `/recipes`, `/showcase`, `/agents`. Agent dumps are generated from `lib/llms.ts`: `/llms.txt`, `/llms-full.txt`, `/agents.md` (drop-in for consuming apps).
